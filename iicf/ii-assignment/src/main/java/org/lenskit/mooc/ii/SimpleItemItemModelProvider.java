...
public class SimpleItemItemModelProvider implements Provider<SimpleItemItemModel> {
    ...
    public SimpleItemItemModel get() {
        ...
        for(Map.Entry<Long, Long2DoubleMap> e:itemVectors.entrySet())
        {
        		Long2DoubleMap similaritys = new Long2DoubleOpenHashMap();
        		for(Map.Entry<Long, Long2DoubleMap> f:itemVectors.entrySet())
        		{
        			long itemId = f.getKey();
        			if(itemId != e.getKey())
        			{
        				double res = Vectors.dotProduct(e.getValue(), f.getValue());
        				res = res / (Vectors.euclideanNorm(e.getValue()) * Vectors.euclideanNorm(f.getValue()));
        				if(res > 0 )
        					similaritys.put(itemId, res);
        			}
        		}
        		itemSimilarities.put(e.getKey(), similaritys);
        }
