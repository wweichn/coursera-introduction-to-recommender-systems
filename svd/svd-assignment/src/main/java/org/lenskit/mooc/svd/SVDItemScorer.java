...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items) {
        ...
        // TODO Store the results in 'results'
    		...
    		RealVector userVec = model.getUserVector(user);
    		RealMatrix itemF = model.getItemFeatureMatrix();
    		RealVector weights = model.getFeatureWeights();
    		
    		RealVector scores = userVec.ebeMultiply(weights);
    		scores = itemF.transpose().preMultiply(scores);
    		
    		for(long item:itemSet)
    		{
    			double bias = baseline.getIntercept() + baseline.getItemBias(item) + baseline.getUserBias(user);
    			int row = model.getItemRow(item);
    			double score = scores.getEntry(row) + bias;
