        ...
        try (ObjectStream<Rating> ratings = dao.query(Rating.class)
            ...
            // TODO Put this user's ratings into the matrix
        		for(Rating r:ratings)
        		{
        			...
        			int column = itemIndex.getIndex(r.getItemId());
        			double baseScore = baseline.getIntercept() + baseline.getItemBias(r.getItemId()) + baseline.getUserBias(r.getUserId());
        			matrix.setEntry(row, column, r.getValue() - baseScore);
        		}
