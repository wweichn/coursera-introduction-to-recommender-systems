    ...
    public AssociationModel get() {
        ...
        for (Long2ObjectMap.Entry<LongSortedSet> xEntry: itemUsers.long2ObjectEntrySet()) {
            ...
            for (Long2ObjectMap.Entry<LongSortedSet> yEntry: itemUsers.long2ObjectEntrySet()) {
                ...
                // TODO Compute P(Y & X) / (P(X) * P(Y)) and store in itemScores
                int count = 0;
                LongBidirectionalIterator it = yUsers.iterator();
                while(it.hasNext())
                {
                		if(xUsers.contains(it.next()))
                			count ++;
                }

                itemScores.put(yId, (float)count * allUsers.size()/(xUsers.size() * yUsers.size()));
            }
